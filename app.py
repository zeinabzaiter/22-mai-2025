...
with tabs[2]:
    ...
    if selected_ab.upper().startswith("VAN"):
        tests_semaine['Alarme'] = tests_semaine[selected_ab] > 0
    else:
...
with tabs[3]:
    ...
    fig_other = px.line(other_ab, x='Semaine', y=selected_other_ab,
                        title=f"% Résistance - {selected_other_ab}", markers=True,
    ...
    fig_other.add_scatter(x=alertes_other['Semaine'], y=alertes_other[selected_other_ab],
                          mode='markers', marker=dict(color='red', size=10), name='Alarme')
